...
class RepositoryModule {
    ...
    internal fun provideRepositoryRemote(@Named(Constants.NAME_REMOTE)
         dataSourceRemote: DataSource<List<DataModel>>) = RepositoryImplementation(dataSourceRemote)

    @Provides
    @Singleton
    @Named(Constants.NAME_LOCAL)
    internal fun provideRepositoryLocal(@Named(Constants.NAME_LOCAL)
